...
downloadable file, store it via :mod:`bublik.core.ai_chat_storage` and record a
:class:`bublik.data.models.AiChatFile` row.

This tool is chat-only: unlike the shared ``MCP_TOOLS`` it needs the run's
thread/user (via ``RunContext[AiChatDeps]``), which the MCP server does not
...
context, enforce the size limit, persist the bytes and build the download URL.
"""
...
__all__ = ['AiChatDeps', 'download_url', 'generate_file']
...
def _max_file_size() -> int:
    return getattr(settings, 'AI_CHAT_FILE_MAX_SIZE', _DEFAULT_MAX_SIZE)
...
@sync_to_async
def _persist(deps: AiChatDeps, filename: str, content_type: str, data: bytes) -> AiChatFile:
    ...
    file_id = uuid.uuid4()
    key = ai_chat_storage.chat_file_key(str(thread.pk), str(file_id), filename)
    ai_chat_storage.upload_bytes(key, data, content_type)
    return AiChatFile.objects.create(
        id=file_id,
        thread=thread,
        filename=filename,
...
async def generate_file(
    ...
    return {
        'file_id': str(ai_chat_file.id),
        'filename': ai_chat_file.filename,
        'content_type': ai_chat_file.content_type,
        'size': ai_chat_file.size,
        'download_url': download_url(ai_chat_file.id),
    }
