...
# This is an idle timeout, not a run deadline: every key of a live run is
# re-expired by `touch_run` (see `append_event` and the streaming heartbeat), so
# a run lasting longer than the TTL does not lose its own bookkeeping.
_EVENT_TTL = 60 * 60

# How often a live run refreshes its keys. A quarter of the TTL leaves three
# missed beats of slack before anything can expire under a stalled worker.
HEARTBEAT_INTERVAL_S = _EVENT_TTL // 4
...
def _refresh(pipe, run_id: str, thread_id: str | None) -> None:
    """Queue TTL refreshes for every key belonging to a live run.

    The thread pointer and the meta hash are only *set* by `register_run`, so
    without this a run outliving `_EVENT_TTL` would lose the pointer that
    `active_run` and cancellation resolve through, and the status that
    `stream_run_events` polls -- mid-run.
    """
    pipe.expire(_run_key(run_id), _EVENT_TTL)
    pipe.expire(_meta_key(run_id), _EVENT_TTL)
    if thread_id:
        pipe.expire(_thread_key(thread_id), _EVENT_TTL)


async def touch_run(run_id: str, thread_id: str | None = None) -> None:
    """Refresh a live run's keys without writing an event.

    Drives the streaming heartbeat, which covers runs that go quiet for longer
    than the TTL (a stalled provider or a long-running tool).
    """
    async with _client().pipeline(transaction=True) as pipe:
        _refresh(pipe, run_id, thread_id)
        await pipe.execute()


async def append_event(run_id: str, sse: str, thread_id: str | None = None) -> None:
    """Append one encoded AG-UI SSE event to the run's buffer."""
    client = _client()
    async with client.pipeline(transaction=True) as pipe:
        pipe.xadd(_run_key(run_id), {DATA_FIELD: sse})
        _refresh(pipe, run_id, thread_id)
        await pipe.execute()
