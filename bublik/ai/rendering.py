...
# formula. Cell values come from the model, so a value like "=1+1" would be
# stored by openpyxl as a live formula (``cell.data_type == 'f'``) and
# evaluated on open; a leading apostrophe forces literal text instead.
_FORMULA_PREFIXES = ('=', '+', '-', '@', '\t', '\r')


def _sanitize_cell(value: object) -> object:
    """Neutralize spreadsheet formula injection in a model-authored cell value.

    Only ``str`` values are touched, so numbers keep their type. A numeric
    string is left alone too: a leading ``-``/``+`` there is a sign, not a
    formula, and quoting it would turn a number into text in the spreadsheet.
    """
...
def _render_xlsx(rows: list[list], title: str) -> bytes:
    ...
    for row in _sanitize_rows(rows):
        sheet.append(row)
    for cell in sheet[1]:
...
def render(
    ...
    if file_format == 'csv' and content:
        # Model-authored CSV text: re-parse and re-emit so its cells go through
        # the same formula sanitization as the `rows` path above.
        return _render_csv(list(csv.reader(io.StringIO(content))))
