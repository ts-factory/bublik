...
def _thread_owner_id(thread_id: str) -> int | None:
    ...
    try:
        return (
            AiChatThread.objects.filter(pk=thread_id).values_list('user_id', flat=True).first()
        )
...
@sync_to_async
def get_owned_file(file_id: str, user_id: int) -> AiChatFile | None:
    """The user's AiChatFile with the given id, or ``None``.
    ...
    try:
        return (
            AiChatFile.objects.select_related('thread')
