...
async def _run_chat(request: Request) -> Response:  # noqa: PLR0911 - endpoint validation exits
    ...
    thread, _created = await sync_to_async(AiChatThread.objects.get_or_create)(
        pk=thread_id,
    ...
    try:
        # Persist the input up front: the run may end without reaching
        # on_complete, and produce_run then persists whatever the model
        # produced on top of this.
    ...
    # The run is already registered as `running`, so everything from here to
    # `spawn_run` has to release it on failure: otherwise the thread stays
    ...
    try:
        ...
        await run_store.finish_run(run_id, 'error')
        return JSONResponse({'detail': 'Unable to start the chat run.'}, status_code=500)
