...
* :class:`Compactor` is a per-run ``ProcessHistory`` history processor
  ...
  ``AiChatThread.context_state`` so later turns (which resend the full
...
* :func:`make_usage_reporter` builds the ``on_complete`` callback for
  ...
  persists it to ``AiChatThread.context_state`` (the *next* run's threshold
...
# `RunContext`/`AiChatDeps` must be importable when the hint is evaluated.
...
from bublik.ai.types import AiChatDeps, CompactionConfig  # noqa: TC001
...
def _read_state(thread_id: str) -> dict:
    ...
    state = (
        AiChatThread.objects.filter(id=thread_id)
        .values_list('context_state', flat=True)
        .first()
    )
    return state or {}
...
def _write_state(thread_id: str, updates: dict) -> None:
    ...
    """
    thread = AiChatThread.objects.filter(id=thread_id).first()
    if thread is None:
        return
...
# Upper bound on the room held back for the model's reply, matching opencode's
# COMPACTION_BUFFER. A model with a huge output cap should not lose that much of
# its window to the reserve.
DEFAULT_RESERVED = 20_000
...
class Compactor:
    ...
    lives in ``AiChatThread.context_state``, so the lru-cached agent stays
    ...
    async def _process(
        ...
        if occupancy < self._usable():
            return working

        return await self._compact(ctx, working, covered)

    def _usable(self) -> float:
        """Tokens of context usable before compaction has to run.

        Modelled on opencode's ``usable()``: hold back enough room for the
        model's reply rather than a flat fraction of the window. On a model with
        a large window and a small output cap, a fraction wastes most of the
        difference. The configured fraction remains the fallback for models
        whose output limit is unknown.
        """
        reserved = self._config.reserved
        if reserved is None and self._output_limit:
            reserved = min(DEFAULT_RESERVED, self._output_limit)
        if reserved is None:
            return self._config.threshold * self._context_limit
        return max(0, self._context_limit - reserved)
...
# Providers disagree on whether the cache buckets are part of `input_tokens`.
# Anthropic reports `cache_creation_input_tokens`/`cache_read_input_tokens`
# *alongside* `input_tokens`, so they have to be added back to get the real
# occupancy. OpenAI-flavored APIs nest `cached_tokens` inside `prompt_tokens`
# (genai-prices calls these "inclusive parent/child buckets"), so adding them
# again counts every cached token twice -- which, with prompt caching on,
# roughly doubles the reported context of every request after the first.
_CACHE_OUTSIDE_INPUT = 'anthropic'

# A provider figure this far above the local estimate is not measuring the same
# thing we are. `estimate_tokens` sees neither the system prompt nor the tool
# schemas, so a small thread's true context is legitimately several times its
# estimate -- hence a deliberately generous factor, and a floor below which the
# ratio is too noisy to judge.
_IMPLAUSIBLE_FACTOR = 20
_MIN_TRUSTED_TOKENS = 50_000


def raw_usage(usage) -> dict:
    """The provider's own token breakdown, kept for diagnosis.

    Mirrors opencode's ``Usage.native``: when a gateway reports something we
    cannot make sense of, the numbers it actually sent are the only way to tell
    what happened.
    """
    fields = ('input_tokens', 'output_tokens', 'cache_read_tokens', 'cache_write_tokens')
    native = {name: getattr(usage, name, None) or 0 for name in fields}
    details = getattr(usage, 'details', None)
    if details:
        native['details'] = dict(details)
    return native


def context_tokens_from_usage(usage, provider_type: str) -> int:
    """Context occupied by one request, per that provider's usage accounting."""
    tokens = (usage.input_tokens or 0) + (usage.output_tokens or 0)
    if _CACHE_OUTSIDE_INPUT in (provider_type or '').lower():
        tokens += (usage.cache_read_tokens or 0) + (usage.cache_write_tokens or 0)
    return tokens


def _implausible_reason(tokens: int, context_limit: int | None, estimate: int) -> str | None:
    """Why ``tokens`` cannot be a real context occupancy, or ``None`` if it can.

    Two independent checks. A request cannot occupy more than the window it fit
    in. And it cannot be orders of magnitude above the text we actually sent --
    which is what catches a gateway whose reported usage is multiplied by the
    number of stream chunks, since that stays under the window on short threads.
    """
    if context_limit and tokens > context_limit:
        return f'above the model context window of {context_limit}'
    if tokens > _MIN_TRUSTED_TOKENS and tokens > _IMPLAUSIBLE_FACTOR * max(estimate, 1):
        return f'{tokens // max(estimate, 1)}x the {estimate}-token estimate of the history'
    return None
...
def make_usage_reporter(
    ...
    the last model request's real token usage to ``AiChatThread.context_state``
    ...
    async def on_complete(result: AgentRunResult) -> AsyncIterator[CustomEvent]:
        ...
        if usage is None:
            return
        # Current context occupancy: what the last request processed plus what
        # it produced, counted the way this provider reports it.
        tokens = context_tokens_from_usage(usage, provider_type)
        if tokens <= 0:
            return
        native = raw_usage(usage)
        implausible = _implausible_reason(
            tokens,
            context_limit,
            estimate_tokens(result.all_messages()),
        )
        if implausible:
            # Persisting a bogus figure pins the thread above the compaction
            # threshold forever, summarizing a short conversation on every turn.
            # Keep the last good value; the `estimate_tokens` floor in
            # `_process` still guards real growth.
            logger.warning(
                'thread %s: %s/%s reported %d context tokens (%s); ignoring it. '
                'Raw usage: %r. If this provider reports cumulative usage on '
                'every stream chunk, set the model setting '
                '`openai_continuous_usage_stats` for it.',
                thread_id,
                provider_id,
                model_id,
                tokens,
