...
class FormulaInjectionTest(SimpleTestCase):
    """Model-authored cells must never reach a spreadsheet as live formulas."""

    DANGEROUS = ('=1+1', '+1+1', '-1+1', '@SUM(A1)', '\tcmd', '\rcmd')

    def _first_column(self, rows):
        workbook = load_workbook(io.BytesIO(render('xlsx', None, rows, 'sheet')))
        return list(workbook.active.iter_rows(min_col=1, max_col=1))

    def test_xlsx_stores_formula_like_values_as_text(self):
        rows = [['header'], *([value] for value in self.DANGEROUS)]
        cells = self._first_column(rows)[1:]
        for value, (cell,) in zip(self.DANGEROUS, cells, strict=True):
            with self.subTest(value=value):
                self.assertNotEqual(cell.data_type, 'f')
                self.assertEqual(cell.value, f"'{value}")

    def test_xlsx_keeps_numbers_and_plain_text_untouched(self):
        rows = [['header'], [-5], ['-5'], ['-5.0'], ['plain']]
        values = [cell.value for (cell,) in self._first_column(rows)[1:]]
        self.assertEqual(values, [-5, '-5', '-5.0', 'plain'])

    def test_csv_from_rows_quotes_formula_like_values(self):
        out = render('csv', None, [['header'], ['=1+1'], ['-5']], 'title')
        parsed = list(csv.reader(io.StringIO(out.decode('utf-8'))))
        self.assertEqual(parsed, [['header'], ["'=1+1"], ['-5']])

    def test_csv_from_raw_content_quotes_formula_like_values(self):
        out = render('csv', 'header\n=1+1\n-5', None, 'title')
        parsed = list(csv.reader(io.StringIO(out.decode('utf-8'))))
        self.assertEqual(parsed, [['header'], ["'=1+1"], ['-5']])
