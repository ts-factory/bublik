...
class RunStoreTest(IsolatedAsyncioTestCase):
    ...
    async def _ttls(self, run_id, thread_id):
        client = run_store._client()
        return {
            'run': await client.ttl(run_store._run_key(run_id)),
            'meta': await client.ttl(run_store._meta_key(run_id)),
            'thread': await client.ttl(run_store._thread_key(thread_id)),
        }

    async def _age_keys(self, run_id, thread_id, ttl):
        """Wind every key of the run down to ``ttl`` seconds."""
        client = run_store._client()
        await client.expire(run_store._run_key(run_id), ttl)
        await client.expire(run_store._meta_key(run_id), ttl)
        await client.expire(run_store._thread_key(thread_id), ttl)

    async def test_append_event_refreshes_every_key_of_the_run(self):
        await run_store.register_run('r-ttl', 't-ttl', 7)
        await run_store.append_event('r-ttl', 'data: seed\n\n', 't-ttl')
        await self._age_keys('r-ttl', 't-ttl', 5)

        await run_store.append_event('r-ttl', 'data: next\n\n', 't-ttl')

        # Without the refresh the thread pointer and meta hash would still be
        # counting down from register_run and expire mid-run.
        for key, ttl in (await self._ttls('r-ttl', 't-ttl')).items():
            with self.subTest(key=key):
                self.assertGreater(ttl, 5)

    async def test_append_event_without_thread_id_leaves_pointer_alone(self):
        await run_store.register_run('r-noth', 't-noth', 7)
        await self._age_keys('r-noth', 't-noth', 5)

        await run_store.append_event('r-noth', 'data: x\n\n')

        ttls = await self._ttls('r-noth', 't-noth')
        self.assertGreater(ttls['run'], 5)
        self.assertEqual(ttls['thread'], 5)

    async def test_touch_run_refreshes_a_silent_run(self):
        await run_store.register_run('r-quiet', 't-quiet', 7)
        await run_store.append_event('r-quiet', 'data: seed\n\n', 't-quiet')
        await self._age_keys('r-quiet', 't-quiet', 5)

        # A run that emits nothing for longer than the TTL still keeps its keys.
        await run_store.touch_run('r-quiet', 't-quiet')

        for key, ttl in (await self._ttls('r-quiet', 't-quiet')).items():
            with self.subTest(key=key):
                self.assertGreater(ttl, 5)

    async def test_heartbeat_interval_leaves_slack_before_expiry(self):
        self.assertLess(run_store.HEARTBEAT_INTERVAL_S, run_store._EVENT_TTL)
