...
class AiChatThreadsApiTest(ReloadUrlconfMixin, APITestCase):
    ...
    def test_rename(self):
        self._auth(self.user)
        tid = str(uuid.uuid4())
        AiChatThread.objects.create(id=tid, user=self.user, title='Original')
        resp = self.client.patch(self._url(tid), {'title': 'Renamed'}, format='json')
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(AiChatThread.objects.get(id=tid).title, 'Renamed')

    def test_patch_cannot_replace_server_owned_messages(self):
        self._auth(self.user)
        tid = str(uuid.uuid4())
        original = [{'role': 'assistant', 'parts': [{'type': 'text', 'content': 'trusted'}]}]
        thread = AiChatThread.objects.create(id=tid, user=self.user, messages=original)
    ...
    def test_delete(self):
        self._auth(self.user)
        tid = str(uuid.uuid4())
        AiChatThread.objects.create(id=tid, user=self.user)
        resp = self.client.delete(self._url(tid))
        self.assertEqual(resp.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(AiChatThread.objects.filter(id=tid).exists())
