...
from bublik.data.serializers.docs.config import (
    AllVersionsResponseSerializer,
    AvailableTypesNamesResponseSerializer,
    ConfigVersionSerializer,
)
from bublik.data.serializers.docs.exceptions import ErrorResponseSerializer
...
@extend_schema_view(
    ...
    list=extend_schema(
        ...
        parameters=[
            ...
            OpenApiParameter(
                ...
                location=OpenApiParameter.QUERY,
                description='configuration activity',
            ),
            OpenApiParameter(
                name='version',
                type=OpenApiTypes.STR,
    ...
    create=extend_schema(
        ...
        The configuration is created only if the project does not already
        contain a configuration with the same type-name.
        ''',
        request=ConfigSerializer,
        responses={
            200: OpenApiResponse(
...
class ConfigViewSet(ModelViewSet):
    ...
    @extend_schema(
        ...
        responses={
            ...
            422: OpenApiResponse(
                response=ErrorResponseSerializer,
                description='Invalid configuration type or name',
            ),
        },
        tags=['Configuration'],
    )
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                ...
                description='List of all configuration versions retrieved successfully',
            ),
            404: OpenApiResponse(
                response=ErrorResponseSerializer,
                description='Configuration not found',
    ...
    def all_versions(self, request, *args, **kwargs):
        ...
        versions_serialized = ConfigVersionSerializer(all_config_versions, many=True).data

        response_data = {
            'type': config.type,
            'name': config.name,
            'project': config.project_id,
            'all_config_versions': versions_serialized,
        }

        serializer = AllVersionsResponseSerializer(response_data)
    ...
    @extend_schema(
        ...
        responses={
            200: OpenApiResponse(
                ...
                description='List of available config types and names retrieved successfully',
            ),
        },
        tags=['Configuration'],
    )
    ...
    def available_types_names(self, request):
        ...
        serializer = AvailableTypesNamesResponseSerializer(config_type_names)

        serializer.is_valid()

        return Response(serializer.config_type_names)
