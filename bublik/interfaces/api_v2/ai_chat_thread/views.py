...
class AiChatThreadViewSet(ModelViewSet):
    """
    ...
    queryset = AiChatThread.objects.all()
    serializer_class = AiChatThreadDetailSerializer
    ...
    def get_serializer_class(self):
        if self.action == 'list':
            return AiChatThreadListSerializer
    ...
    def get_queryset(self):
        user = self._get_user()
        if not user:
            return AiChatThread.objects.none()
        return AiChatThread.objects.filter(user=user)

    def _get_owned_or_none(self, pk, user):
        try:
            thread = AiChatThread.objects.filter(pk=pk).first()
    ...
    def list(self, request, *args, **kwargs):
        ...
        serializer = AiChatThreadListSerializer(
            threads,
            many=True,
    ...
    def partial_update(self, request, *args, **kwargs):
        ...
        if thread is None:
            raise NotFound
        serializer = AiChatThreadDetailSerializer(thread, data=request.data, partial=True)
